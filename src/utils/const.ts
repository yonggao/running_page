...
// For security reasons, avoid using the default public token provided by Mapbox as much as possible.
...
// 'mapcn' uses Carto dark-matter — similar to Mapbox dark, free, no token
...
export const MAP_TILE_STYLES = {
  ...
  // MapCN (Carto Basemaps) — free, no token required
  mapcn: {
    'osm-bright':
...
if (typeof window !== 'undefined') {
  // AMap uses its own key system (VITE_AMAP_KEY / VITE_AMAP_SECURITY_KEY)
  if (MAP_TILE_VENDOR === 'amap') {
    console.info(
      '✅ Using AMap (高德地图) - Works reliably in China!\n' +
        '🔑 Keys: VITE_AMAP_KEY and VITE_AMAP_SECURITY_KEY\n' +
        '📚 Configure in Vercel Dashboard → Environment Variables'
    );
  }

  // Validate token requirements for non-amap vendors
  ...
  // Validate style matches vendor (skip for amap — uses MAP_TILE_STYLE, not MAP_TILE_STYLE_LIGHT)
  if (MAP_TILE_VENDOR !== 'amap') {
